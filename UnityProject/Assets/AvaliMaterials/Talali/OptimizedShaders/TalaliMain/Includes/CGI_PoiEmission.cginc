...
{
    float3 emission0 = 0;
    float emissionStrength0 = float(0.3);
    ...
    float3 emissionColor1 = 0;
        ...
        float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
            ...
            {
                emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(15,15,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(1)).rgb * float4(1,1,1,1).rgb;
            }
        #else
        ...
        if (float(1))
        {
            ...
            {
                #if defined(PROP_EMISSIONSCROLLINGCURVE1) || !defined(OPTIMIZER_ENABLED)
                    emissionStrength1 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve1, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve1) + (dot(pos1, float4(0,-10,0,0)) * float(75)) + _Time.x * float(7.5));
                #endif
            }
            else
            {
                emissionStrength1 *= calculateScrollingEmission(float4(0,-10,0,0), float(7.5), float(75), float(30), float(0), pos1);
            }
        }
