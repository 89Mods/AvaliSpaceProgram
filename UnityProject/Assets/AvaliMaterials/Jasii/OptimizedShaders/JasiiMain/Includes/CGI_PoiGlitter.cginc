    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        for (int j = -1; j <= 1; j ++)
        {
            for (int i = -1; i <= 1; i ++)
            {
                ...
                float2 rando = pos;
                pos = 0.5 + 0.5 * sin(float(0.04) * 6.2831 * pos);
        ...
        switch(float(0))
        {
            ...
            if (float(0) == 1 || float(1) != 0)
            {
                ...
                float theta = radians((randomBoy + _Time.x * float(1)) * 360);
                float cs = cos(theta);
                float sn = sin(theta);
        ...
        if(float(0) == 1 || float(1) != 0 && !float(0))
        {
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(2), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(2) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(2) * glitterTexture.a * glitterMask;
        }
    }
#endif
